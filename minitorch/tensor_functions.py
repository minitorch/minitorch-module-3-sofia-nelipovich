...
class Mul(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        # Производная по a: grad_output * b
        # Производная по b: grad_output * a
...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        minus = out.f.add_zip(minitorch.tensor([1.0]), out.f.neg_map(out))
        return grad_output.f.mul_zip(grad_output, out.f.mul_zip(out, minus))
...
class LT(Function):
    ...
    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        # По производной сравнения градиент всегда 0
        a, b = ctx.saved_values
        return a - a, b - b
...
class Permute(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        # Обратная перестановка осей
        inverse_order = [order_tuple.index(i) for i in range(len(order_tuple))]
